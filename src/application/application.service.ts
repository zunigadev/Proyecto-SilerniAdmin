...
export class ApplicationService {
    ...
    async changeStatusApplication(id: number, statusApplication: UpdateStatusChildDto) {
        try {
            ...
            await this.prisma.$transaction(async (prisma: PrismaClient) => {
                ...
                if (statusApplication.status === StatusApplication.ACCEPTED) {
                    const postulationChildCredential = await this.credentialService.generateCredentialsToStudent();
      ...
      private async createCredentialsAndLinkToTutor(prisma: PrismaClient, idDataTutor: number) {
